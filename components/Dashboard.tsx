...
const Dashboard: React.FC<DashboardProps> = ({ entries }) => {
  ...
  const payrollData = useMemo(() => {
    // Fix: Use a generic with reduce to ensure correct type inference for the accumulator.
  ...
  const qualityData = useMemo(() => {
    ...
    const defectsByPosition = filteredEntries.reduce<Record<string, number>>((acc, entry) => {
        if(entry.defectQuantity > 0) {
            acc[entry.jobPosition] = (acc[entry.jobPosition] || 0) + entry.defectQuantity;
        }
        return acc;
    }, {});
    return Object.entries(defectsByPosition)
      .map(([jobPosition, totalDefects]) => ({ jobPosition, totalDefects }))
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-2xl rounded-xl p-6 md:p-8">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-1">Owner's Dashboard</h2>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">High-level overview of factory performance.</p>

      {/* Date Filter */}
      <div className="flex flex-wrap gap-4 mb-8 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="startDate" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Start Date</label>
          ...
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="endDate" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">End Date</label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-gray-50 dark:bg-gray-900/50 p-6 rounded-lg">
           <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Payroll Report</h3>
           <div className="max-h-96 overflow-y-auto">
             <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400 sticky top-0">
                    <tr>
                        <th scope="col" className="px-4 py-2">Worker Name</th>
                        <th scope="col" className="px-4 py-2 text-right">Total Pay (Ks)</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    {payrollData.length === 0 && (
                        <tr><td colSpan={2} className="text-center p-4">No data for selected period.</td></tr>
                    )}
                </tbody>
             </table>
           </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Productivity by Shift</h3>
            ...
            {/* Quality Control */}
            <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Defects by Job Position</h3>
                {totalDefects > 0 ? (
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-6 h-64">
                        ...
                        <ul className="text-xs space-y-1">
                            {qualityData.map((item, index) => (
                                <li key={item.jobPosition} className="flex items-center">
                                    <span className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: COLORS[index % COLORS.length] }}></span>
                                    <span className="font-medium text-gray-800 dark:text-gray-200">{item.jobPosition}:</span>
                    ...
                    <div className="h-64 flex items-center justify-center text-gray-500 dark:text-gray-400">No defects recorded.</div>
                )}
            </div>
        </div>
      </div>
